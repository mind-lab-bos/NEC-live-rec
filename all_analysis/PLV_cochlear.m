...
cycles = 31; % determines resolution of wavelet
...
%% Topo
topoSlice = squeeze(mean(aggregateChannelPLVs(1,:,:,16),[1 2]));
figure;
topoplot(topoSlice, EEG.chanlocs,  'electrodes', 'labelpoint', 'chaninfo', EEG.chaninfo); 
%%
avgPLVs = squeeze(mean(aggregateChannelPLVs(:,:,:,16),[1 4]));
...
figure;
t = tiledlayout('flow');
t.Title.String = 'Phase locking Topos at 4.2 Hz';
...
figure;
t = tiledlayout('flow');
t.Title.String = 'Live Vs. Rec PLV at 4.2 Hz';
t.Title.FontWeight = 'bold';
%frontal cluster channels: [1 2 3 4 6 7 8 9 12 24 25 26 27 28 29]

nexttile;
topoplot((topoSlice_LiveFast+topoSlice_LiveSlow)/2, EEG.chanlocs,'plotchans',[],'style','map','electrodes','ptslabels','chaninfo', EEG.chaninfo);  %, 'whitebk', 'on'
title('Live');
clim(color_lim);
nexttile;
topoplot((topoSlice_RecFast+topoSlice_RecSlow)/2, EEG.chanlocs, 'plotchans',[],'style','map','electrodes','ptslabels', 'chaninfo', EEG.chaninfo); 
title('Recorded');
clim(color_lim);
set(gcf,"color","white")
cb = colorbar;
...
figure;
t = tiledlayout('flow');
t.Title.String = 'Fast Vs. Slow PLV at 4.2 Hz';
t.Title.FontWeight = 'bold';
%frontal cluster channels: [1 2 3 4 6 7 8 9 12 24 25 26 27 28 29]
...
avgLiveFast = squeeze(aggregateChannelPLVs(:,1,4,12));
avgLiveSlow = squeeze(aggregateChannelPLVs(:,2,4,12));
avgRecFast = squeeze(aggregateChannelPLVs(:,3,4,12));
avgRecSlow = squeeze(aggregateChannelPLVs(:,4,4,12));
...
avgLiveFast = squeeze(mean(aggregatePLVs(:,1,5),3));
avgLiveSlow = squeeze(mean(aggregatePLVs(:,2,5),3));
avgRecFast = squeeze(mean(aggregatePLVs(:,3,5),3));
avgRecSlow = squeeze(mean(aggregatePLVs(:,4,5),3));
A = zeros(42,2); %% row factor is tempo f/s, column factor is live/rec
A(1:21,1) = avgLiveFast;
A(1:21,2) = avgRecFast;
A(22:42,1) = avgLiveSlow;
